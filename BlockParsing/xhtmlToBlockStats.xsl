...
<xsl:template match="(html|body|p|table|tr|td|div|dd|dt|dl|ul|ol|li|br|nobr)[descendant::text()]">
	<!-- link text -->
	<xsl:variable name="textLinkCount" select="count(descendant::a[descendant::text()][(ancestor::html|ancestor::body|ancestor::p|ancestor::table|ancestor::tr|ancestor::td|ancestor::div|ancestor::dd|ancestor::dt|ancestor::dl|ancestor::ul|ancestor::ol|ancestor::li|ancestor::br|ancestor::nobr)[last()] = current()])"/>
	<xsl:variable name="linkText"><xsl:for-each select="descendant::a[(ancestor::html|ancestor::body|ancestor::p|ancestor::table|ancestor::tr|ancestor::td|ancestor::div|ancestor::dd|ancestor::dt|ancestor::dl|ancestor::ul|ancestor::ol|ancestor::li|ancestor::br|ancestor::nobr)[last()] = current()]//text()"><xsl:value-of select="concat(.,' ')"/></xsl:for-each></xsl:variable>
	...
	<xsl:if test="1=1">
	<!-- ($textLinkCount = 1) or ($linkTextWordCount div $textLinkCount &gt; 3) or ($linkTextLength div $linkTextWordCount &gt; 9) or ($nonLinkTextLength div $linkTextLength &gt; 1 and $linkTextLength &gt; 0) -->
