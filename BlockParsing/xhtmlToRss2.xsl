...
<xsl:template match="(body|p|table|tr|td|div|dd|dt|dl|ul|ol|li|br|nobr)[descendant::text()]">
	<xsl:variable name="tlc" select="count(descendant::a[descendant::text()])"/>
	<xsl:if test="$tlc &gt; 0">
		<xsl:variable name="lt"><xsl:for-each select="descendant::a/descendant::text()"><xsl:value-of select="concat(.,' ')"/></xsl:for-each></xsl:variable>
		...
		<xsl:if test="($awpl &gt; 1 and $rnlt &gt; 1) or ($awpl &gt; 2 and $rnlt &gt; 0.2) or ($awpl &gt; 3) or ($alwl &gt; 11)">
			<xsl:apply-templates/>
		</xsl:if>
	</xsl:if>
</xsl:template>

<xsl:template match="a[descendant::text()]">
	...
	<xsl:variable name="nlt">
		<xsl:choose>
			...
			<xsl:when test="(ancestor::html|ancestor::body|ancestor::p|ancestor::table|ancestor::tr|ancestor::td|ancestor::div|ancestor::dd|ancestor::dt|ancestor::dl|ancestor::ul|ancestor::ol|ancestor::li|ancestor::br|ancestor::nobr)[count(descendant::a[@href != current()/@href]) = 0][last()]">
				1<xsl:value-of select="(ancestor::html|ancestor::body|ancestor::p|ancestor::table|ancestor::tr|ancestor::td|ancestor::div|ancestor::dd|ancestor::dt|ancestor::dl|ancestor::ul|ancestor::ol|ancestor::li|ancestor::br|ancestor::nobr)[count(descendant::a[@href != current()/@href]) = 0][last()]/descendant-or-self::text()[not(self::* = current())]"/>
			</xsl:when>
			...
			<xsl:when test="following-sibling::node()[1][text()][string-length(self::text()) &gt; 5]">
	...
	<xsl:variable name="tlc" select="1"/>
	<xsl:variable name="ltwc"><xsl:call-template name="word-count"><xsl:with-param name="text" select="$lt"/></xsl:call-template></xsl:variable>
	<xsl:variable name="ltl" select="string-length(normalize-space(translate($lt,'.,?!-:;)( ','')))"/>
	<xsl:variable name="nltl" select="string-length(normalize-space(translate($nlt,'.,?!-:;)( ','')))"/>
	<xsl:variable name="awpl"><xsl:value-of select="$ltwc div $tlc"/></xsl:variable>
		...
		<item>
			...
			<title>
				<xsl:choose>
					...
					<xsl:otherwise><xsl:value-of select="$nlt"/></xsl:otherwise>
				</xsl:choose>
			</title>
			<description>
				<xsl:choose>
					<xsl:when test="$lt != ''"><xsl:value-of select="$nlt"/></xsl:when>
